...
<div align="center">
	...
	<%--mysql_select.jsp --%>
<%=request.getParameter("giveup") %>
<% 
String[] giveup=(String[])request.getParameterValues("giveup");
String[] sessionc_id2=(String[])request.getSession().getAttribute("sessionc_id2");
String[] sessionday2=(String[])request.getSession().getAttribute("sessionday2");
String[] sessions_time2=(String[])request.getSession().getAttribute("sessions_time2");
String[] sessionborrower2=(String[])request.getSession().getAttribute("sessionborrower2");
for (int i=0;i<giveup.length;i++) { 
    out.println(giveup[i]); //逐个输出数组元素的值 
    out.println(sessionc_id2[Integer.parseInt(giveup[i])]); 
    out.println(sessionday2[Integer.parseInt(giveup[i])]); 
    out.println(sessions_time2[Integer.parseInt(giveup[i])]);
    out.println(sessionborrower2[Integer.parseInt(giveup[i])]); %>
    <p></p>
    <% 
	...
	String SQ="delete from classroom_request where c_id='"+sessionc_id2[Integer.parseInt(giveup[i])]
			+"' and day="+sessionday2[Integer.parseInt(giveup[i])].replace("-","")+" and s_time="+sessions_time2[Integer.parseInt(giveup[i])].replace(":","")
			+" and borrower='"+sessionborrower2[Integer.parseInt(giveup[i])]+"'";//更新classroom_request;
