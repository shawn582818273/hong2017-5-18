...
<div align="center">
	...
	<%--mysql_select.jsp --%>
<%//request.getParameter("refuse") %>
<% 
String[] refuse=(String[])request.getParameterValues("refuse");
String[] sessionc_id1=(String[])request.getSession().getAttribute("sessionc_id2");
String[] sessionday1=(String[])request.getSession().getAttribute("sessionday2");
String[] sessions_time1=(String[])request.getSession().getAttribute("sessions_time2");
String[] sessionborrower1=(String[])request.getSession().getAttribute("sessionborrower2");
for (int i=0;i<refuse.length;i++) { 
    out.println(refuse[i]); //逐个输出数组元素的值 
    out.println(sessionc_id1[Integer.parseInt(refuse[i])]); 
    out.println(sessionday1[Integer.parseInt(refuse[i])]); 
    out.println(sessions_time1[Integer.parseInt(refuse[i])]);
    out.println(sessionborrower1[Integer.parseInt(refuse[i])]); %>
    <p></p>
    <% 
    String strConn="jdbc:mysql://localhost:3306/hong";  
	String strUser="root"; 
	String strPassword="123456";
	String SQ="UPDATE classroom_request SET isborrowed = 0 WHERE c_id='"+sessionc_id1[Integer.parseInt(refuse[i])]
	+"' and day="+sessionday1[Integer.parseInt(refuse[i])].replace("-","")+" and s_time="+sessions_time1[Integer.parseInt(refuse[i])].replace(":","")
	+" and borrower='"+sessionborrower1[Integer.parseInt(refuse[i])]+"'";
	String SQ1="UPDATE classroom_state SET canbo = 1 ,borrower=0 WHERE c_id='"+sessionc_id1[Integer.parseInt(refuse[i])]
	+"' and day="+sessionday1[Integer.parseInt(refuse[i])].replace("-","")+" and s_time="+sessions_time1[Integer.parseInt(refuse[i])].replace(":","")
	+" and borrower='"+sessionborrower1[Integer.parseInt(refuse[i])]+"'";
	String SQ2="UPDATE boardroom_state SET canbo = 1 ,borrower=0 WHERE b_id='"+sessionc_id1[Integer.parseInt(refuse[i])]
	+"' and day="+sessionday1[Integer.parseInt(refuse[i])].replace("-","")+" and s_time="+sessions_time1[Integer.parseInt(refuse[i])].replace(":","")
	+" and borrower='"+sessionborrower1[Integer.parseInt(refuse[i])]+"'";
