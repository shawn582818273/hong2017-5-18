...
<form id="form11" name="form11" method="post" action="manager_examined2.jsp" onsubmit="return check()">
<%try{
...
String strSql="SELECT * from classroom_request where isborrowed=1 order by request_t";  
...
out.print("<tr><td>房间号：</td><td>日期:</td><td>开始时间：</td><td>借用人：</td><td>请求时间：</td><td>是否借成功：</td><td>理由：</td><td>驳回请求</td></tr>");
while(rs.next())
{
	out.print("<tr>");
	...
	out.print("<td>"+"<input type='checkbox' name='refuse' value="+i+" /></td>");
	out.print("</tr>");i++;
}
out.print("</table>");
out.print("</div>");
