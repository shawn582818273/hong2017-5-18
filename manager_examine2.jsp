...
<% 
String[] approval=(String[])request.getParameterValues("approval");
String[] sessionc_id1=(String[])request.getSession().getAttribute("sessionc_id1");
String[] sessionday1=(String[])request.getSession().getAttribute("sessionday1");
String[] sessions_time1=(String[])request.getSession().getAttribute("sessions_time1");
String[] sessionborrower1=(String[])request.getSession().getAttribute("sessionborrower1");
for (int i=0;i<approval.length;i++) { 
    out.println(approval[i]); //逐个输出数组元素的值 
    out.println(sessionc_id1[Integer.parseInt(approval[i])]); 
    out.println(sessionday1[Integer.parseInt(approval[i])]); 
    out.println(sessions_time1[Integer.parseInt(approval[i])]);
    out.println(sessionborrower1[Integer.parseInt(approval[i])]); %>
    <p></p>
    <% 
	...
	String SQ="UPDATE classroom_request SET isborrowed = 1 WHERE c_id='"+sessionc_id1[Integer.parseInt(approval[i])]
	+"' and day="+sessionday1[Integer.parseInt(approval[i])].replace("-","")+" and s_time="+sessions_time1[Integer.parseInt(approval[i])].replace(":","")
	+" and borrower='"+sessionborrower1[Integer.parseInt(approval[i])]+"'";//更新classroom_request;
	
	String SQ1="UPDATE classroom_state SET canbo = 0 , borrower='"
	+sessionborrower1[Integer.parseInt(approval[i])]+"' WHERE c_id='"
	+sessionc_id1[Integer.parseInt(approval[i])]
	+"' and day="+sessionday1[Integer.parseInt(approval[i])].replace("-","")
	+" and s_time="+sessions_time1[Integer.parseInt(approval[i])].replace(":","");//更新classroom_state;
	
	String SQ2="UPDATE boardroom_state SET canbo = 0 , borrower='"
			+sessionborrower1[Integer.parseInt(approval[i])]+"' WHERE b_id='"
			+sessionc_id1[Integer.parseInt(approval[i])]
			+"' and day="+sessionday1[Integer.parseInt(approval[i])].replace("-","")
			+" and s_time="+sessions_time1[Integer.parseInt(approval[i])].replace(":","");//更新boardroom_state;
	
	String SQ3="delete from classroom_request where c_id='"+sessionc_id1[Integer.parseInt(approval[i])]
			+"' and day="+sessionday1[Integer.parseInt(approval[i])].replace("-","")+" and s_time="+sessions_time1[Integer.parseInt(approval[i])].replace(":","")
			+" and borrower!='"+sessionborrower1[Integer.parseInt(approval[i])]+"'";//更新classroom_request;
			
	String SQ4="delete from classroom_request where c_id='null'";
	...
	stat.executeUpdate(SQ3);
	stat.executeUpdate(SQ4);
	//关闭连接    
	stat.close();  
	conn1.close();
	%>
